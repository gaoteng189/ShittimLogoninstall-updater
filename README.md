...
## WinUI 3 发送端（.NET 10 + Windows App SDK）
...
├── MainWindow.xaml / .xaml.cs 界面
├── AssemblyInfo.cs            产品名 / 版本 / 版权 / 平台声明
├── FileServer.cs              SLU/1 协议服务端（对应 src/file_server.cpp）
├── Crc32.cs                   CRC-32，与 C++ 端位级一致
└── app.manifest               DPI 感知 + Windows 10/11 兼容性
```
...
- **分发文件**：默认自动识别 exe 同目录的 `ShittimLogon.zip`，也可手动选择。
  客户端按相对路径请求这个文件；所在目录内的其它文件同样可被请求（与 C++ 版一致）
- **监听端口**：默认 50304，可改。启动失败（例如端口被占）会在状态栏和日志里
  说明具体原因，而不是静默失败
- **实时日志**：每个客户端的连接、请求、结果都带时间戳列出；
  被拒绝的请求会写明理由
- **当前传输**：正在发送的文件、进度、速度单独一行，不往日志里刷屏
- **统计**：当前连接数、已完成传输次数、累计发送字节数

### 路径安全

`FileServer.cs` 的 `TryResolvePath` 与 `src/file_server.cpp` 的 `ResolveRequestPath`
逐条对应，双重防护：

1. 按路径组件净化：`/` 归一化为 `\`；拒绝绝对路径与 UNC；拒绝盘符路径；
   拒绝 `..` 组件；拒绝含 `:` 的组件（顺带挡住 NTFS 备用数据流）
2. 把结果规范化成绝对路径，确认它确实落在共享目录之内

```text
请求 ../secret.txt        → 拒绝：不允许使用 .. 进行目录穿越
请求 C:\Windows\win.ini   → 拒绝：不允许使用盘符路径
请求 sub/../../x          → 拒绝：不允许使用 .. 进行目录穿越
```

### 跨实现验证

C# 服务端与 C++ 客户端做过二进制级互通测试：

```text
C++ 客户端 → C# 发送端，拉取 51.2 MB
exit code 0
SHA-256   源 = 拉取 = 79C0E77F…3A3A7      完全一致
解压      201 个文件
回环吞吐  268.4 MB/s（CRC32 6857B7C4）

请求 ../secret.txt   → 拒绝：不允许使用 .. 进行目录穿越
请求 不存在的文件    → 文件不存在 — 请求的文件不存在
```

这轮验证还抓出了一处与 C++ 实现不一致的地方：客户端在地址只写到 `host:port` 时
会发 `nameLength = 0` 表示「要服务端准备好的默认文件」，而最初的 C# 实现把它当成
非法请求拒掉了。C++ 服务端对此的处理是回退到配置的默认文件名，并在响应头里返回
**实际提供的文件名**（客户端据此落盘），C# 侧已对齐。
